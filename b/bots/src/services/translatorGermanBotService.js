// src/services/translatorGermanBotService.js
...
function getUserSettings(chatId) {
  if (!userSettings.has(chatId)) {
    userSettings.set(chatId, { direction: 'de-ru', lastTranslation: null });
  }
  return userSettings.get(chatId);
}

function getLanguageCodes(settings) {
  return settings.direction === 'de-ru'
    ? { source: 'de', target: 'ru' }
    : { source: 'ru', target: 'de' };
}
...
bot.onText(/\/switch/, (msg) => {
  ...
  settings.direction = settings.direction === 'de-ru' ? 'ru-de' : 'de-ru';
  const emoji = getLanguageEmoji(settings);
...
bot.onText(/\/speak (.+)/, async (msg, match) => {
  ...
  try {
    ...
    await bot.sendAudio(chatId, audioPath, {
      caption: `🔊 Озвучка на ${langs.source === 'de' ? 'немецком' : 'русском'} языке`
    });
  } catch (e) {
    console.error(e);
...
bot.on('voice', async (msg) => {
  ...
  try {
    ...
    await bot.sendAudio(chatId, audioPath, {
      caption: `🔊 Озвучка на ${langs.target === 'de' ? 'немецком' : 'русском'} языке`
    });
  } catch (e) {
    console.error(e);
...
bot.on('callback_query', async (query) => {
  ...
  if (query.data.startsWith('speak_')) {
    ...
    await bot.sendAudio(chatId, audioPath, {
      caption: `🔊 Озвучка перевода на ${langs.target === 'de' ? 'немецком' : 'русском'} языке`
    });
  }
...
console.log('[BOT] Немецкий переводчик запущен');
