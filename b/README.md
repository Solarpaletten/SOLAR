...
Тестовая база данных `schema_t.prisma` **не использует миграции**, а создаётся через:
```bash
npx prisma db push --schema=prisma/schema_t.prisma --force-reset
```

Это обеспечивает:
- Чистую тестовую среду при каждом запуске CI
- Отсутствие проблем с историей миграций в тестовой базе
- Быструю синхронизацию схемы из актуальной модели

При локальной разработке используйте:
```bash
# Создание тестовой БД или обновление схемы
