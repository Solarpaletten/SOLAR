// 🎈 Family-Friendly Sidebar для уставших мам-бухгалтеров
// Включает детскую зону, темы и социальные функции
...
const FamilyFriendlySidebar: React.FC = () => {
  ...
  // 🌙 Автоматическая темная тема вечером (забота о глазах)
  useEffect(() => {
  ...
  return (
    <div className={`w-64 h-screen flex flex-col transition-all duration-300 ${
      ...
      <NavLink
        ...
        className={`block p-4 text-2xl font-bold transition-colors ${
      ...
      <div className={`border-t p-3 ${isDarkMode ? 'border-gray-700' : 'border-[#165468]'}`}>
        ...
        >
          {isDarkMode ? '☀️ Светлая тема' : '🌙 Темная тема для глаз'}
        </button>
      </div>
...
export default FamilyFriendlySidebar;