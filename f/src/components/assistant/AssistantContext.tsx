...
export const AssistantProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [preferences, setPreferences] =
    useState<AssistantPreferences>(defaultPreferences);

  const [userLanguage, setUserLanguageState] = useState<AssistantLanguage>(
    AssistantLanguage.ENGLISH
  );
  const [targetLanguage, setTargetLanguageState] = useState<AssistantLanguage>(
      ...
      switch (data.type) {
        ...
        case 'NEW_MESSAGE':
          ...
          if (preferences.voiceEnabled && data.message.senderType !== 'user') {
            const textToSpeak =
              data.message.translatedContent || data.message.originalContent;
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch(
        `/api/assistant/sessions?${queryParams.toString()}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
