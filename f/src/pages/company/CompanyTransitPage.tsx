...
const CompanyTransitPage: React.FC = () => {
  ...
  const getCompanyInfo = (id: string | null) => {
    // РЕАЛЬНЫЕ данные из API тестов - обновлено с правильными ID!
    const companies = [
      { id: 1, name: 'Desert Solar DMCC', schema: 'company_1' },
      { id: 2, name: 'Emirates Energy', schema: 'company_2' },
      { id: 3, name: 'Test Energy Company', schema: 'company_3' },
      { id: 15, name: 'ASSET BILANS', schema: 'company_15' }, // ✅ Добавлена компания ID 15!
    ];
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // ✅ ИСПРАВЛЕНО: Используем правильные ключи localStorage (как в axios.ts)
      localStorage.setItem('currentCompanyId', companyId); // ✅ camelCase как в axios.ts
      localStorage.setItem('currentCompanyName', companyInfo.name);

      console.log(`✅ Saved to localStorage: currentCompanyId = ${companyId}`);
      console.log(`✅ Saved to localStorage: currentCompanyName = ${companyInfo.name}`);
        ...
        <div className="text-center">
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-4 p-3 bg-gray-50 rounded text-xs text-left">
              ...
              <p>localStorage Key: currentCompanyId = {companyId}</p>
            </div>
          )}
        </div>
      </div>
